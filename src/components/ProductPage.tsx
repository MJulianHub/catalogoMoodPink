...
const ProductPage = () => {
  ...
  const similar = shuffle(products.filter((p) => p.id !== product.id)).slice(0, 4);
  ...
  return (
    <Container className="my-5">
          ...
          <Carousel>
            {product.images.map((img, idx) => (
              <Carousel.Item key={idx}>
                <img src={img} alt={`${product.name} ${idx + 1}`} className="d-block w-100" style={{ objectFit: 'cover', height: '100%' }} />
              </Carousel.Item>
            ))}
          </Carousel>
        </Col>
        <Col md={6}>
          ...
          <div className="mt-3">
            <Button
              as="a"
              href={
                `https://wa.me/50585696708?text=${encodeURIComponent(
      ...
      <Row className="g-3 row-cols-2 row-cols-md-3 row-cols-lg-4">
        {similar.map((p) => (
          <Col key={p.id}>
            <Card onClick={() => navigate(`/product/${p.id}`)} className="border-0 h-100 shadow-sm" style={{ cursor: 'pointer' }}>
